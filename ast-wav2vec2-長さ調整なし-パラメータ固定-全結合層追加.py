...
from model import Model_wav2vec, AST_Vector_not, CombinedClassifier_1layer_add, CombinedClassifier_2layer_add
...
if __name__ == '__main__':
    ...
    ast_model_vector160 = AST_Vector_not(ast_model, embed_size=768)
    #ast_model_vector160 =ast_model_vector160.to(device)
    
    wav2vec_model = Model_wav2vec(args,device)

    mix_model = CombinedClassifier_2layer_add(ast_model_vector160, wav2vec_model)
    ...
    for epoch in range(num_epochs):        
        ...
        torch.save(mix_model.state_dict(), os.path.join(
            '/home/haruto/SSL_Anti-spoofing/models/ast-wav2vec2-パラメータ固定-正しい-2つドロップアウトなし全結合層追加', 'epoch_{}.pth'.format(epoch)))